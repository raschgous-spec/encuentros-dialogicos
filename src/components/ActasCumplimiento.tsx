...
interface CumplimientoRow {
  ...
  cumplimientoTotal: number; // % with all 4 encounters
}
...
export const ActasCumplimiento = () => {
  ...
  const [estAutorizados, setEstAutorizados] = useState<Array<{ correo: string; sede: string; facultad: string; programa: string }>>([]);
  const [actas, setActas] = useState<Array<{ estudiante_id: string; momento: string }>>([]);
    ...
    const load = async () => {
      try {
        const [p, ea, ac] = await Promise.all([
          paginate('profiles', 'id, email'),
          paginate('estudiantes_autorizados', 'correo, sede, facultad, programa'),
  ...
  // Build per-student acta moment sets
  ...
  const stats = useMemo((): CumplimientoRow[] => {
    ...
    groups.forEach(({ sede, facultad, programa, correos }) => {
      ...
      result.push({
        sede, facultad, programa, totalAutorizados: total,
        conEncuentro1, conEncuentro2, conEncuentro3, conEncuentro4,
        cumplimientoTotal: total > 0 ? Math.round((conTodos / total) * 100) : 0,
      });
    });
  ...
  const sedeStats = useMemo(() => {
    ...
    map.forEach((v, k) => {
      const allComplete = stats.filter(s => s.sede === k).reduce((sum, s) => {
        return sum + (s.totalAutorizados > 0 ? Math.round(s.cumplimientoTotal * s.totalAutorizados / 100) : 0);
      }, 0);
  ...
  const facultadStats = useMemo(() => {
    ...
    map.forEach((v, k) => {
      ...
      const allComplete = relatedPrograms.reduce((sum, s) => {
        return sum + (s.totalAutorizados > 0 ? Math.round(s.cumplimientoTotal * s.totalAutorizados / 100) : 0);
      }, 0);
      v.cumplimientoTotal = v.totalAutorizados > 0 ? Math.round((allComplete / v.totalAutorizados) * 100) : 0;
    });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Verifica cuántos estudiantes han generado actas en cada encuentro, agrupado por sede, facultad y programa</p>
      </div>
      ...
      <div className="flex gap-2">
        {(['sede', 'facultad', 'programa'] as const).map(level => (
          <Button
            key={level}
            variant={expandedLevel === level ? 'default' : 'outline'}
            size="sm"
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Detalle de Cumplimiento - {expandedLevel === 'sede' ? 'Por Sede' : expandedLevel === 'facultad' ? 'Por Facultad' : 'Por Programa'}</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {displayData.map((row, i) => (
                  <tr key={`${row.sede}-${row.facultad}-${row.programa}-${i}`} className={i % 2 === 0 ? 'bg-muted/50' : ''}>
                    <td className="py-2 px-2 text-xs">{row.sede}</td>
