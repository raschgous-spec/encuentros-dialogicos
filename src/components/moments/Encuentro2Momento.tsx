...
export const Encuentro2Momento = ({ onComplete, isLocked = false }: Encuentro2MomentoProps) => {
  ...
  const generatePDF = (data: z.infer<typeof planFormSchema>) => {
    ...
    yPos += 8;
    doc.setFontSize(14);
    doc.text('MOMENTO 4 - ENCUENTRO 2', 105, yPos, { align: 'center' });
    yPos += 12;
    ...
    if (user) {
      let sigY = yPos + 15;
      await addSignaturesToPDF(doc, user.id, 'encuentro2', sigY);
    }

    doc.save(`plan-mejoramiento-encuentro2-${new Date().toISOString().split('T')[0]}.pdf`);
  };
              ...
              {user && (
                <Card>
                  ...
                  <CardContent className="space-y-6">
                    <SignaturePad userId={user.id} momento="encuentro2" role="estudiante" label="Firma del Estudiante" isLocked={isLocked} />
                    <SignaturePad userId={user.id} momento="encuentro2" role="coordinador" label="Firma del Coordinador" isLocked={isLocked} />
                  </CardContent>
                </Card>
              )}
            </TabsContent>
